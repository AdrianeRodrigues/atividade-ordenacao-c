...
void partition(campos cp[], int lb, int ub, int *j){
	int down, up;
	campos cp_aux, temp;
	cp_aux = cp[lb];
	up = ub;
...
void quicksort(campos cp[], int lb, int ub){
    int j = -1;
    if(lb >= ub)
        return;                     
    partition(cp, lb, ub, &j);      
	quicksort(cp, lb, j-1);          
	quicksort(cp, j+1, ub);
}
